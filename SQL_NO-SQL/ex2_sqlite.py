...
request_1 = 'SELECT Orders.OrderID FROM Orders, Products INNER JOIN Customers ON Orders.CustomerID = Customers.CustomerID INNER JOIN [Order Details] ON [Order Details].OrderID = Orders.OrderID AND Products.ProductID = [Order Details].ProductID WHERE Customers.CustomerID = "ALFKI" GROUP BY Orders.OrderID HAVING COUNT(Products.CategoryID) >= 2'
